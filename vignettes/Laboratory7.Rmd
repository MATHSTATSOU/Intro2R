...
```{r}
library(ggplot2)
g <- ggplot(NULL, aes(x=c(-4,4))) + stat_function(geom="line",fun = "dt", args=list(df=9), color = "blue") 
...
This is a similar result for  the case of a  hypergeometric distribution. Where the variance of $Y = \sum_{i=1}^n Y_i$ is $Var(Y)=npq\frac{N-n}{N-1}$, when we have replacement then the hypergeometric becomes that of a binomial and $Var(Y) = npq$.
...
Please note that when using `RMD` you must make sure that all code does not require user input such as `locator() ` or makes a new window `windows()`.
