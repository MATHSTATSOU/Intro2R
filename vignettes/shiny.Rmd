...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    sidebarPanel(
      ...
      fileInput("file", "Select csv file:", accept = c(".csv")),
      checkboxInput("header", "The file has a header!", TRUE),
      br(),

      selectInput("var1", "Select quant./qual. variable for x-axis:", ""),
      selectInput("var2", "Select quant. variable for y-axis:", ""),
...
server <- function(input, output, session) {
  ...
  reactive({
    req(input$file)
    input_data = read.csv(input$file$datapath, header = input$header)

    updateSelectInput(session, "var1", choices = names(input_data))
    updateSelectInput(session, "var2", choices = names(input_data))
  })
  ...
  output$plot1 <- renderPlot({
    ...
    df <- input_data[,c(input$var1, input$var2)]
