...
We will take an example. Suppose that the dangerous levels of DDT in the flesh of fish is  5ppm.
...
Eat <-with(ddt, cut(DDT, breaks = c(0,5, max(DDT)+1), labels = c("OK", "Dang"), right = FALSE))
...
where $odds = \frac{p}{1-p}$ -- notice that $p = \frac{exp(odds)}{1+exp(odds)}$
